...
def parse_prd(file_path):
    ...
    for section_name, section_content in section_pairs:
        ...
        logger.debug(f"Section content (first 200 chars):\n{section_content[:200]}...")
        ...
        if section_name == 'overview':
            ...
            if not things:
                logger.warning("No things found. Attempting fallback parsing...")
                # Fallback: Try splitting by lines starting with '- <Name>:'
                thing_lines = [line for line in section_content.split('\n') if line.strip().startswith('- ') and ':' in line]
                things = []
                for line in thing_lines:
                    match = re.match(r'^- (\w+): (.*?)$', line, re.DOTALL)
                    if match:
                        things.append((match.group(1), match.group(2)))
            ...
            for thing_name, thing_content in things:
                ...
                logger.debug(f"Thing content (first 200 chars):\n{thing_content[:200]}...")
                ...
                # Extract properties
                properties = []
                prop_section = re.search(r'## Properties:\s*((?:- \w+:.*?\n)+)', thing_content, re.DOTALL)
                if prop_section:
                    ...
                    # Fallback: Look for properties without header
                ...
                if rules_section:
                    ...
                    # Fallback: Look for rules without header
                ...
                if actions_section:
                    ...
                    # Fallback: Look for actions without header
